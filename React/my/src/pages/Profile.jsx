    ...
    <div className="crop-modal-overlay active" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="crop-modal">
        ...
        <div className="crop-area-wrapper">
          <div className="crop-canvas-container" style={{ aspectRatio: type === "avatar" ? "1" : "2/1" }}>
            <canvas ref={canvasRef} onMouseDown={handleMouseDown} style={{ width: "100%", height: "100%", cursor: "grab" }} />
          </div>
...
function Toast({ message, onDone }) {
  ...
  return (
    <div className="toast-notification active">
      <i className="bx bx-check-circle" /><span>{message}</span>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════
   SHARE MODAL
   TODO: Connect to real profile URL when backend is ready
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   BLOCK MODAL
   TODO: Connect to backend API to actually block user
   TODO: Remove this component when backend is NOT ready — keep UI only
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   REPORT MODAL
   TODO: Connect to backend API to submit report
   TODO: Remove this component when backend is NOT ready — keep UI only
═══════════════════════════════════════════════════════════ */
  ...
  return (
    <div className="photo-viewer-overlay active" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div className="photo-viewer-content">
        {avatarUrl
          ? <img id="photoViewerImg" src={avatarUrl} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%", border: "3px solid rgba(255,255,255,0.15)" }} />
          : <div id="photoViewerAvatar" style={{ width: "100%", height: "100%", borderRadius: "50%", background: "linear-gradient(135deg,#667eea 0%,#764ba2 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 100, fontWeight: "bold", color: "white", border: "3px solid rgba(255,255,255,0.15)" }}>{initials}</div>
        }
      </div>
    </div>
  );
...
/* ═══════════════════════════════════════════════════════════
   ...
   TODO: Connect to backend for real friend request system
   TODO: Remove this component when backend is NOT ready — keep UI only
═══════════════════════════════════════════════════════════ */
function FriendRequestPopup({ isOpen, name, initials, onAccept, onDecline }) {
  if (!isOpen) return null;
  return (
    <div className="friend-request-popup" style={{ display: "flex", flexDirection: "column" }}>
      <div className="friend-req-top" style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <div className="friend-req-actions">
        <button className="friend-req-accept" onClick={onAccept}>✓ Accept</button>
        <button className="friend-req-decline" onClick={onDecline}>✗ Decline</button>
      </div>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════
   MORE DROPDOWN
   TODO: Backend needed for block and report actions
═══════════════════════════════════════════════════════════ */
function MoreDropdown({ isOpen, onBlock, onReport }) {
  if (!isOpen) return null;
  return (
    <div className="more-dropdown active" style={{ position: "fixed", bottom: 80, right: 16, top: "auto" }}>
      <div className="more-dropdown-item block-item" onClick={onBlock}>
        <i className="bx bx-block" /><span>Block User</span>
      </div>
      <div className="more-dropdown-item report-item" onClick={onReport}>
        <i className="bx bx-flag" /><span>Report User</span>
      </div>
    </div>
  );
}
...
export default function Profile() {
  const [profile, setProfile] = useState({ name: "Prabhat Behera", username: "@Prabhat4a", bio: "Computer Science Student | Tech Enthusiast | Coffee Lover ☕" });
  ...
  // TODO: These states are for backend features — remove initial mock values when backend is ready
  ...
  useEffect(() => {
    ...
    const handler = (e) => setMoreDropdownOpen(false);
    document.addEventListener("click", handler);
    return () => document.removeEventListener("click", handler);
  }, [moreDropdownOpen]);

  // TODO: Remove this auto-popup timer when backend is ready — it's just a demo
  useEffect(() => {
  ...
  const handleFileRead = (file, type) => {
    ...
    reader.onload = (e) => setCropModal({ open: true, type, imageUrl: e.target.result });
  ...
  const handleCropApply = (dataUrl) => { if (cropModal.type === "cover") setCoverUrl(dataUrl); else setAvatarUrl(dataUrl); };
  const handleSaveProfile = (data) => { setProfile(data); setEditProfileOpen(false); showToast("Profile updated successfully!"); };
  const handleSaveSocial = (val) => { setSocialValues((prev) => ({ ...prev, [socialModal.type]: val })); setSocialModal({ open: false, type: null }); showToast("Link saved!"); };

  // TODO: Replace with real backend API call when ready
  const handleAddFriend = () => {
    if (isFriends) { if (window.confirm(`Unfriend ${profile.name}?`)) { setIsFriends(false); setRequestPending(false); showToast("Unfriended successfully"); } return; }
    if (requestPending) return;
    setRequestPending(true);
    showToast("Friend request sent!");
    // TODO: Remove this demo popup when backend is ready
    setTimeout(() => setFriendRequestPopupOpen(true), 3000);
  };

  // TODO: Replace with real backend API call when ready
  const handleAcceptFriend = () => { setIsFriends(true); setRequestPending(false); setFriendRequestPopupOpen(false); showToast("You are now friends!"); };
  const handleDeclineFriend = () => { setIsFriends(false); setRequestPending(false); setFriendRequestPopupOpen(false); };

  // TODO: Replace with real backend block API when ready
  ...
  // TODO: Replace with real backend report API when ready
  ...
  return (
    <div className="profile-page">
      ...
      {/* TODO: Remove this toggle button when backend auth is ready — used for demo only */}
      <div style={{ padding: "8px 16px", background: "#111", borderBottom: "1px solid #222", display: "flex", justifyContent: "flex-end" }}>
        <button
          onClick={() => { setViewingOwnProfile(!viewingOwnProfile); setIsBlocked(false); setIsFriends(false); setRequestPending(false); }}
          style={{ padding: "5px 12px", background: "#a78bfa", border: "none", borderRadius: 6, color: "white", cursor: "pointer", fontSize: 12 }}
        >
      ...
      <div className="cover-photo-section">
        <div className="cover-photo-container" onClick={() => viewingOwnProfile && coverFileRef.current.click()}>
      ...
      {/* BLOCKED STATE */}
      {/* TODO: This blocked state will be driven by backend — remove isBlocked local state when ready */}
      {!viewingOwnProfile && isBlocked && (
        <div className="blocked-state">
          <div className="blocked-top-bar">
            <button className="unblock-top-btn" onClick={handleUnblock}>Unblock</button>
          </div>
          <div className="blocked-body">
            <div className="blocked-icon"><i className="bx bx-block" /></div>
            <h2>You've blocked this account</h2>
            <p>Unblock this account to see their profile and connect with them.</p>
          </div>
        </div>
      )}
      ...
      <div className="profile-header-section">
        <div className="profile-info">
          ...
          <p className={`profile-bio${bioExpanded ? " expanded" : ""}`}>{profile.bio}</p>
        ...
        {/* TODO: Connect Add Friend / Message / More to backend */}
        {!viewingOwnProfile && !isBlocked && (
          <div className="profile-buttons">
            <button
            ...
            >
              <i className={`bx ${isFriends ? "bx-user-minus" : requestPending ? "bx-time" : "bx-user-plus"}`} />
              {isFriends ? "Unfriend" : requestPending ? "Pending..." : "Add Friend"}
            </button>
            <button
              className="message-btn"
              disabled={!isFriends}
              style={{ opacity: isFriends ? 1 : 0.5, cursor: isFriends ? "pointer" : "not-allowed" }}
              onClick={() => !isFriends ? showToast("Send a friend request first to message this person") : showToast("Chat coming soon!")}
            >
              <i className="bx bx-message-rounded" /> Message
            </button>
            <button className="more-btn" onClick={(e) => { e.stopPropagation(); setMoreDropdownOpen(!moreDropdownOpen); }}>
          ...
          <div className="social-links-card">
            {socialMeta.map(({ key, label, iconClass, wrapClass }) => (
              <div className="social-link-item" key={key}>
                ...
                {viewingOwnProfile && (
                  <button className="social-edit-btn" onClick={() => setSocialModal({ open: true, type: key })}>
                    <i className="bx bx-pencil" />
                  </button>
                )}
                {socialValues[key] && (
                  <a className="social-go-btn" href={socialValues[key]} target="_blank" rel="noreferrer">
            ...
            <div className="social-link-item">
              ...
              {viewingOwnProfile && (
                <button className="social-edit-btn" onClick={() => setSocialModal({ open: true, type: "email" })}>
                  <i className="bx bx-pencil" />
                </button>
              )}
              {socialValues.email && (
                <a className="social-go-btn" href={`mailto:${socialValues.email}`}>
                  <i className="bx bx-link-external" />
                </a>
              )}
            </div>
      ...
      {/* MORE DROPDOWN — TODO: Backend needed */}
      <MoreDropdown
        isOpen={moreDropdownOpen}
        onBlock={() => { setMoreDropdownOpen(false); setBlockModalOpen(true); }}
        onReport={() => {
          if (hasReported) { showToast("You have already reported this profile"); setMoreDropdownOpen(false); return; }
          setMoreDropdownOpen(false); setReportModalOpen(true);
        }}
      />

      {/* FRIEND REQUEST POPUP — TODO: Backend needed */}
      <FriendRequestPopup
